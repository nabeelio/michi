...
// File lives under src/Segments/Extensions/ (historical layout). Namespace is `Segments.FileSystem`
...
namespace Segments.FileSystem;

/// <summary>
/// Opt-in filesystem extension methods on <see cref="SPath" />. Import with
/// `using Segments.FileSystem;` when filesystem-facing operations are needed. The core `SPath`
...
/// using <see cref="SPath.Value" /> (the OS-native form). No additional normalization is
/// performed -- `SPath.From` already normalized the path at construction time.
/// <para>
...
#region Existence
    ...
    public static bool FileExists(this SPath path)
    {
        Guard.NotNull(path);
    ...
    /// <exception cref="ArgumentNullException">`path` is null.</exception>
    public static bool DirectoryExists(this SPath path)
    {
        Guard.NotNull(path);
...
#region Creation
    ...
    /// <see cref="SPath" /> instance for fluent chaining.
    /// </summary>
    ...
    /// <example>
    ///     <code>
    /// var logs = SPath.From("/var/app/logs").CreateDirectory();
    /// logs.DirectoryExists().ShouldBeTrue();
    ///     </code>
    /// </example>
    public static SPath CreateDirectory(this SPath path)
    {
        Guard.NotNull(path);
    ...
    /// create the path itself. Returns the same <see cref="SPath" /> for fluent chaining.
    ...
    /// </remarks>
    public static SPath EnsureParentExists(this SPath path)
    {
        Guard.NotNull(path);
    ...
    /// than replacing the file. Returns the same <see cref="SPath" /> for fluent chaining.
    /// </summary>
    ...
    /// </para>
    /// </remarks>
    public static SPath CreateOrClearDirectory(this SPath path)
    {
        Guard.NotNull(path);
...
#region Interop
    ...
    /// <exception cref="ArgumentNullException">`path` is null.</exception>
    public static FileInfo ToFileInfo(this SPath path)
    {
        Guard.NotNull(path);
    ...
    /// <exception cref="ArgumentNullException">`path` is null.</exception>
    public static DirectoryInfo ToDirectoryInfo(this SPath path)
    {
        Guard.NotNull(path);
...
#region Enumeration
    ...
    /// <returns>
    /// An <see cref="IEnumerable{T}" /> of <see cref="SPath" /> yielded lazily; ordering is
    ...
    /// </remarks>
    public static IEnumerable<SPath> EnumerateFiles(
        this SPath path,
    ...
    private static IEnumerable<SPath> EnumerateFilesIterator(
        SPath path,
        string searchPattern,
        SearchOption searchOption
    )
    ...
    /// the underlying BCL enumeration on first `MoveNext`.
    /// </exception>
    public static IEnumerable<SPath> EnumerateDirectories(
        this SPath path,
        string searchPattern,
        SearchOption searchOption
    )
    ...
    private static IEnumerable<SPath> EnumerateDirectoriesIterator(
        SPath path,
        string searchPattern,
        SearchOption searchOption
    )
...
#region Deletion
    ...
    /// </remarks>
    public static void DeleteFile(this SPath path)
    {
        Guard.NotNull(path);
    ...
    /// </para>
    /// </remarks>
    public static void DeleteDirectory(this SPath path, bool recursive = true)
    {
        Guard.NotNull(path);
...
#region Move and copy
    ...
    )
    {
        ...
        var sourceIsDirectory = Directory.Exists(source.Value);
        if (sourceIsDirectory && IsDescendantPath(source, resolvedDest)) {
            throw new IOException(
                $"Cannot move directory '{source.Value}' to '{resolvedDest.Value}': destination is inside the source directory."
            );
        }
    ...
    /// <see cref="MoveTo(SPath, SPath, ExistsPolicy)" /> with two differences: the source
    ...
    /// <param name="policy">
    /// Conflict policy. Default <see cref="ExistsPolicy.Fail" /> -- any existing file or
    /// directory at the resolved target throws. See <see cref="MoveTo(SPath, SPath, ExistsPolicy)" />
    ...
    /// copy. The argument is the SOURCE-side <see cref="SPath" /> of the entry (file or
    ...
    /// be a pure predicate over the source <see cref="SPath" />; exceptions thrown by the
    ...
    /// <para>
    /// Same TOCTOU caveat as <see cref="MoveTo(SPath, SPath, ExistsPolicy)" />: the
    ...
    )
    {
        ...
        if (sourceIsDirectory && IsDescendantPath(source, resolvedDest)) {
            throw new IOException(
                $"Cannot copy directory '{source.Value}' to '{resolvedDest.Value}': destination is inside the source directory."
            );
        }
...
#region Move/copy helpers (shared)
    ...
    private static SPath ResolveDestination(SPath source, SPath destination)
    {
    ...
    // Host-OS-correct equality. SPath.Equals already routes through HostOs.PathComparer,
    ...
    private static bool PathsEqual(SPath a, SPath b) => a.Equals(b);

    private static bool IsDescendantPath(SPath ancestor, SPath candidate)
    {
        var ancestorPath = ancestor.ToUnixString();
        var candidatePath = candidate.ToUnixString();
        if (candidatePath.Length <= ancestorPath.Length) {
            return false;
        }

        if (!candidatePath.AsSpan(0, ancestorPath.Length).Equals(ancestorPath.AsSpan(), HostOs.PathComparison)) {
            return false;
        }

        return ancestorPath[^1] == '/' || candidatePath[ancestorPath.Length] == '/';
    }

    private static void MoveFileWithPolicy(SPath source, SPath dest, ExistsPolicy policy)
    {
    ...
    private static void MoveDirectoryWithPolicy(SPath source, SPath dest, ExistsPolicy policy)
    {
        if (File.Exists(dest.Value)) {
    ...
    private static void MergeDirectoryInto(SPath source, SPath dest, ExistsPolicy policy)
    {
    ...
    private static void RemoveIfEmpty(SPath path)
    {
        if (!Directory.Exists(path.Value)) {
            return;
        }
    ...
    private static void CopyFileWithPolicy(SPath source, SPath dest, ExistsPolicy policy)
    {
        dest.EnsureParentExists();
