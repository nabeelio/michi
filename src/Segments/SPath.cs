...
// Alias to disambiguate from the new instance-level Path property on SPath.
...
/// </summary>
/// <remarks>
/// Construct via <see cref="From(string, SPathOptions?)" /> or
/// <see cref="TryFrom(string?, out SPath?, SPathOptions?)" />.
...
public sealed class SPath : IEquatable<SPath>, IComparable<SPath> {
    ...
    /// SPath via substring operations that preserve invariants.
    /// </summary>
    private SPath(string normalizedPath, string root)
    {
...
#region Construction
    ...
    /// <summary>
    /// Normalizes `path` and returns an <see cref="SPath" />.
    /// Relative inputs are resolved against <see cref="SPathOptions.BaseDirectory" />.
    /// </summary>
    ...
    /// </exception>
    public static SPath From(string path, SPathOptions? options = null)
    {
        Guard.NotNull(path, "Path cannot be null. Use TryFrom to accept null input without exceptions.");

        var result = PathNormalizer.Normalize(path, options ?? SPathOptions.Default);
    ...
    /// for relative-path resolution (overriding <see cref="SPathOptions.BaseDirectory" />).
    /// </summary>
    /// <exception cref="ArgumentNullException">
    ...
    /// Path cannot be normalized to a valid absolute path.
    /// </exception>
    public static SPath From(string path, string relativeTo, SPathOptions? options = null)
    {
        Guard.NotNull(path, "Path cannot be null. Use TryFrom to accept null input without exceptions.");
        Guard.NotNull(
            relativeTo,
        ...
        var result = PathNormalizer.Normalize(path, options ?? SPathOptions.Default, relativeTo);
    ...
    /// </summary>
    public static bool TryFrom(string? path, [NotNullWhen(true)] out SPath? result, SPathOptions? options = null)
    {
        if (path is null) {
            result = null;
    ...
    /// <summary>
    /// Non-throwing counterpart to <see cref="From(string, string, SPathOptions?)" />.
    /// Returns `true` and sets `result` if `path` can be resolved against `relativeTo`;
    /// returns `false` with null result otherwise.
    /// </summary>
    public static bool TryFrom(
        string? path,
        string? relativeTo,
        [NotNullWhen(true)] out SPath? result,
        SPathOptions? options = null
    )
    {
        if (path is null || relativeTo is null) {
            result = null;
        ...
        try {
            result = From(path, relativeTo, options);
    ...
    /// <see cref="SPath" />. Uses <see cref="CultureInfo.InvariantCulture" /> so numeric and
    ...
    /// </exception>
    public static SPath Format(string template, params object?[] args)
    {
        Guard.NotNull(
            template,
...
#region String output
    ...
    /// <summary>
    /// Explicitly converts this path to its OS-native separator string form.
    /// Equivalent to <see cref="ToString" />.
    /// </summary>
    /// <exception cref="ArgumentNullException">
    /// `path` is null.
    /// </exception>
    public static explicit operator string(SPath path)
    {
        Guard.NotNull(path);
...
#region Well-known paths
    ...
    /// </summary>
    public static SPath Home => WellKnownPaths.Home.Value;

    /// <summary>
    /// System temporary directory. Computed once at first access.
    /// </summary>
    public static SPath Temp => WellKnownPaths.Temp.Value;
    ...
    /// </remarks>
    public static SPath InstalledDirectory => WellKnownPaths.InstalledDirectory.Value;
    ...
    /// </remarks>
    public static SPath ApplicationData => WellKnownPaths.ApplicationData.Value;
    ...
    /// files directly here. Computed once at first access.
    /// </remarks>
    public static SPath LocalApplicationData => WellKnownPaths.LocalApplicationData.Value;
    ...
    /// files directly here. Computed once at first access.
    /// </remarks>
    public static SPath CommonApplicationData => WellKnownPaths.CommonApplicationData.Value;

    /// <summary>
    /// Process current working directory. Evaluated on every access -- the process CWD is mutable.
    /// </summary>
    public static SPath CurrentDirectory => From(Directory.GetCurrentDirectory());
...
#region Equality, comparison, operators
    ...
    /// <inheritdoc />
    public override bool Equals(object? obj) => obj is SPath other && Equals(other);

    /// <summary>
    /// Hash code from the same source as <see cref="Equals(SPath?)" /> -- consistency is structural.
    /// </summary>
    public override int GetHashCode() => HostOs.PathComparer.GetHashCode(_path);

    /// <summary>
    /// Lexicographic ordering consistent with <see cref="Equals(SPath?)" />. Null precedes any value.
    /// </summary>
    ...
    /// </summary>
    public static bool operator ==(SPath? left, SPath? right)
    {
    ...
    /// </summary>
    public static bool operator !=(SPath? left, SPath? right) => !(left == right);
    ...
    /// `null` sorts before any path.
    /// </summary>
    public static bool operator <=(SPath? left, SPath? right) => left is null || left.CompareTo(right) <= 0;

    /// <summary>
    /// Returns `true` when `left` sorts after `right`. Any path sorts
    /// after `null`.
    /// </summary>
    public static bool operator >(SPath? left, SPath? right) => left is not null && left.CompareTo(right) > 0;
...
#region Navigation
    ...
    /// This path is a root.
    /// </exception>
    public SPath Parent => TryGetParent(out var parent) ? parent! : throw new NoParentException(this);

    /// <summary>
    /// Non-throwing parent lookup. Returns `false` at root.
    /// </summary>
    /// <param name="parent">
    /// On success, receives the parent <see cref="SPath" />. On failure (this path is a
    /// root) receives `null`.
    /// </param>
    /// <returns>
    /// `true` if a parent was produced; `false` when this path is a root and has no parent.
    /// </returns>
    public bool TryGetParent(out SPath? parent)
    {
        if (_path == Root) {
            parent = null;
...
#region Joining
    ...
    /// </exception>
    public static SPath operator /(SPath left, string segment)
    {
        ...
        var result = PathNormalizer.Normalize(combined, SPathOptions.Default);
    ...
    /// </param>
    /// <returns>
    /// A new <see cref="SPath" /> with the segments appended and re-normalized. Returns
    /// this instance unchanged if `segments` is null, empty, or contains only null/empty entries.
    /// </returns>
    public SPath Join(params string[]? segments)
    {
        ...
        var result = PathNormalizer.Normalize(sb.ToString(), SPathOptions.Default);
...
#region Containment
    ...
    /// </param>
    /// <returns>
    /// A new <see cref="SPath" /> that is either equal to this path or a descendant of it.
    /// </returns>
    /// <exception cref="ArgumentNullException">
    /// `segment` is null.
    /// </exception>
    ...
    /// </exception>
    [Pure]
    public SPath ResolveContained(string segment)
    {
        Guard.NotNull(
            segment,
    ...
    ///     `segment` stays loud, mirroring <see cref="TryFrom(string?, out SPath?, SPathOptions?)" />'s contract.
    ...
    /// </param>
    /// <param name="result">
    /// On success, the resolved contained <see cref="SPath" />; on failure,
    /// `null`.
    /// </param>
    /// <returns>
    ...
    /// `segment` is null.
    /// </exception>
    public bool TryResolveContained(string segment, [NotNullWhen(true)] out SPath? result)
    {
        Guard.NotNull(
            segment,
            "Segment cannot be null. Pass a non-null relative path fragment; use TryResolveContained for empty/invalid input handling."
    ...
    /// <see cref="TryResolveContained(string, out SPath?)" /> so both methods use the same
    ...
    {
        // Strip leading separators so absolute-looking input still resolves under the base.
        ...
        try {
            normalized = PathNormalizer.Normalize(stripped, SPathOptions.Default, _path);
...
#region Mutation
    ...
    /// <summary>
    /// Returns a new <see cref="SPath" /> with the final segment replaced by `name`.
    ...
    /// </param>
    /// <returns>
    /// A new <see cref="SPath" /> sharing this path's parent chain but with its final
    ...
    /// </exception>
    public SPath WithName(string name)
    {
    ...
    /// <summary>
    /// Returns a new <see cref="SPath" /> with the file extension replaced. Leading dot is optional;
    ...
    /// </exception>
    public SPath WithExtension(string? extension)
    {
    ...
    /// <summary>
    /// Returns a new <see cref="SPath" /> with any trailing extension stripped. Idempotent.
    /// </summary>
    public SPath WithoutExtension() => HasExtension ? WithName(NameWithoutExtension) : this;
