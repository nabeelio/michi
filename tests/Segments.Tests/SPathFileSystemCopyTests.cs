...
//   2. The optional `filter` parameter (Func<SPath, bool>?) is invoked once per
...
public sealed class SPathFileSystemCopyTests : IDisposable {
    private readonly string _tempRoot;

    public SPathFileSystemCopyTests()
    {
        _tempRoot = Path.Combine(Path.GetTempPath(), $"segments-fs-copy-{Guid.NewGuid():N}");
    ...
    {
        var src = WriteFile("a.txt", "hello");
        var dst = SPath.From(TempPath("b.txt"));
    ...
    public void CopyTo_preserves_source_after_directory_copy()
    {
        ...
        srcDir.CopyTo(SPath.From(TempPath("dst-tree")));
    ...
    public void CopyTo_directory_Fail_policy_throws_on_collision()
    {
        ...
        Should.Throw<IOException>(() => srcDir.CopyTo(SPath.From(TempPath("dst-tree"))));
    ...
    public void CopyTo_directory_DirectoryReplace_at_resolved_target_replaces_tree()
    {
        ...
        srcDir.CopyTo(SPath.From(TempPath("dst-tree")), ExistsPolicy.DirectoryReplace);
    ...
    public void CopyTo_directory_DirectoryMerge_merges_file_contents()
    {
        ...
        srcDir.CopyTo(SPath.From(TempPath("dst-tree")), ExistsPolicy.DirectoryMerge);
    ...
    public void CopyTo_directory_MergeAndOverwrite_overwrites_same_named_files()
    {
        ...
        srcDir.CopyTo(SPath.From(TempPath("dst-tree")), ExistsPolicy.MergeAndOverwrite);
    ...
    public void CopyTo_directory_MergeAndSkip_preserves_existing_dest_files()
    {
        ...
        srcDir.CopyTo(SPath.From(TempPath("dst-tree")), ExistsPolicy.MergeAndSkip);
    ...
    public void CopyTo_directory_MergeAndOverwriteIfNewer_only_overwrites_newer_sources()
    {
        ...
        srcDir.CopyTo(SPath.From(TempPath("dst-tree")), ExistsPolicy.MergeAndOverwriteIfNewer);
    ...
    public void CopyTo_directory_into_existing_file_throws_IOException()
    {
        ...
        var ex = Should.Throw<IOException>(() => srcDir.CopyTo(SPath.From(TempPath("dst-parent"))));
    ...
    [Fact]
    public void CopyTo_same_directory_path_is_noop()
    {
        var srcDir = MakeDir("src-tree");
        WriteFile("src-tree/a.txt", "a");
        var invocations = 0;

        Should.NotThrow(() => srcDir.CopyTo(
                    SPath.From(srcDir.Value),
                    filter: _ => {
    ...
    [Fact]
    public void CopyTo_directory_rejects_destination_inside_source()
    {
        var srcDir = MakeDir("src-tree");
        WriteFile("src-tree/a.txt", "a");

        var ex = Should.Throw<IOException>(() => srcDir.CopyTo(
                    SPath.From(TempPath("src-tree/child")),
                    filter: _ => false
                )
        );
