...
//   2. Elements are SPath instances produced via SPath.From, not raw strings.
...
public sealed class SPathFileSystemEnumerationTests : IDisposable {
    private readonly string _tempRoot;

    public SPathFileSystemEnumerationTests()
    {
        _tempRoot = Path.Combine(Path.GetTempPath(), $"segments-fs-enum-{Guid.NewGuid():N}");
...
#region EnumerateFiles
    ...
    // Elements must be SPath instances, not raw strings. We verify by calling an
    // SPath-only method on each element.
    [Fact]
    public void EnumerateFiles_returns_SPath_instances()
    {
        var root = SPath.From(_tempRoot);
        File.WriteAllText(Path.Combine(_tempRoot, "a.txt"), "a");
    ...
    public void EnumerateFiles_throws_eagerly_on_null_path()
    {
        Should.Throw<ArgumentNullException>(() =>
                SPathExtensions.EnumerateFiles(null!, "*", SearchOption.TopDirectoryOnly)
        );
    }
    ...
    {
        var missing = SPath.From(Path.Combine(_tempRoot, "does-not-exist"));
...
#region EnumerateDirectories
    ...
    [Fact]
    public void EnumerateDirectories_returns_SPath_instances()
    {
        var root = SPath.From(_tempRoot);
    ...
    {
        Should.Throw<ArgumentNullException>(() => SPathExtensions.EnumerateDirectories(
                    null!,
                    "*",
