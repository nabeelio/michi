...
// they return the same SPath instance so callers can chain further operations.
public sealed class SPathFileSystemCreationTests : IDisposable {
    private readonly string _tempRoot;

    public SPathFileSystemCreationTests()
    {
        _tempRoot = Path.Combine(Path.GetTempPath(), $"segments-fs-create-{Guid.NewGuid():N}");
...
#region CreateDirectory
    ...
    {
        var target = SPath.From(Path.Combine(_tempRoot, "new-dir"));
    ...
    {
        var target = SPath.From(Path.Combine(_tempRoot, "twice"));
    ...
    {
        var target = SPath.From(Path.Combine(_tempRoot, "a", "b", "c"));
    ...
    // Fluent-chaining contract: the returned SPath is the same reference as the input.
    [Fact]
    public void CreateDirectory_returns_same_instance()
    {
        var target = SPath.From(Path.Combine(_tempRoot, "same-ref"));
    ...
    public void CreateDirectory_on_existing_file_throws_IOException()
    {
        ...
        var target = SPath.From(filePath);
...
#region EnsureParentExists
    ...
    public void EnsureParentExists_creates_parent_only()
    {
        ...
        var child = SPath.From(Path.Combine(parent, "child.txt"));
    ...
    {
        var child = SPath.From(Path.Combine(_tempRoot, "child.txt"));
    ...
    // rather than letting SPath.Parent throw NoParentException.
    [Fact]
    public void EnsureParentExists_is_noop_at_root()
    {
        ...
        var root = SPath.From(rootString);
...
#region CreateOrClearDirectory
    ...
    {
        var target = SPath.From(Path.Combine(_tempRoot, "fresh"));
    ...
    {
        var target = SPath.From(Path.Combine(_tempRoot, "has-files"));
    ...
    {
        var target = SPath.From(Path.Combine(_tempRoot, "nested"));
    ...
    {
        var target = SPath.From(Path.Combine(_tempRoot, "keep-root"));
        Directory.CreateDirectory(target.Value);
    ...
    {
        var target = SPath.From(Path.Combine(_tempRoot, "same-ref-clear"));
