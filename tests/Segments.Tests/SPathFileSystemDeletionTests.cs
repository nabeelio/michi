...
public sealed class SPathFileSystemDeletionTests : IDisposable {
    private readonly string _tempRoot;

    public SPathFileSystemDeletionTests()
    {
        _tempRoot = Path.Combine(Path.GetTempPath(), $"segments-fs-delete-{Guid.NewGuid():N}");
...
#region DeleteFile
    ...
    {
        var target = SPath.From(Path.Combine(_tempRoot, "does-not-exist.txt"));
...
#region DeleteDirectory
    ...
    // We do not assert message wording (it belongs to the BCL, not to Segments).
    [Fact]
    ...
    {
        var target = SPath.From(Path.Combine(_tempRoot, "missing-dir"));
    ...
    {
        var target = SPath.From(Path.Combine(_tempRoot, "missing-dir-nonrec"));
