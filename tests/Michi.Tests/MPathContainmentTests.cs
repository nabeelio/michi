...
public sealed class MPathContainmentTests {
    ...
    // Reusable ZIP-slip corpus. Backslash traversal is split into a Windows-only set because
    // `\\` is a separator on Windows but a filename character on Unix.
    ...
    // Windows-only traversal corpus. On Unix, `\\` is a filename character rather than a separator.
    ...
    public void ResolveContained_handles_UNC_segment_against_local_base_on_windows()
    {
        ...
        // A UNC-looking segment is still rejected when it resolves outside the base.
        var basePath = MPath.From(@"C:\app");
