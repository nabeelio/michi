...
// but the round-trip correctness matters -- constructing an MPath from the returned
// FullName must produce an MPath equal to the original on every TFM and platform.
public sealed class MPathFileSystemInteropTests : IDisposable {
    private readonly string _tempRoot;

    public MPathFileSystemInteropTests()
    {
        _tempRoot = Path.Combine(Path.GetTempPath(), $"michi-fs-interop-{Guid.NewGuid():N}");
...
#region ToFileInfo
    ...
    public void ToFileInfo_does_not_require_file_to_exist()
    {
        ...
        var path = MPath.From(missingPath);
    ...
    // Round-trip: MPath.From(info.FullName) must produce an MPath equal to the original.
    // Uses the existing host-OS comparer semantics (case-insensitive on Windows/macOS,
    // case-sensitive on Linux) via the MPath equality contract.
    [Fact]
    public void ToFileInfo_roundtrip_preserves_MPath_equality()
    {
        var original = MPath.From(Path.Combine(_tempRoot, "roundtrip.txt"));

        var info = original.ToFileInfo();
        var reconstructed = MPath.From(info.FullName);
...
#region ToDirectoryInfo
    ...
    public void ToDirectoryInfo_returns_DirectoryInfo_with_matching_FullName()
    {
        ...
        var path = MPath.From(dirPath);

        var info = path.ToDirectoryInfo();

        info.ShouldNotBeNull();
        // DirectoryInfo.FullName may or may not include a trailing separator depending on
        // platform and the constructor input. We compare against MPath.Value directly:
        // MPath normalization is already settled, and the test validates FullName *matches*
        // that canonical form.
        info.FullName.ShouldBe(path.Value);
    }
    ...
    public void ToDirectoryInfo_does_not_require_directory_to_exist()
    {
        ...
        var path = MPath.From(missingDir);
    ...
    [Fact]
    public void ToDirectoryInfo_roundtrip_preserves_MPath_equality()
    {
        var original = MPath.From(Path.Combine(_tempRoot, "roundtrip-dir"));
